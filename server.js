...
function buildSystemPrompt(business, menuItems, customer) {
  const menuText = menuItems
    .map((i) => `- ${i.name}: $${i.price.toLocaleString("es-CL")}${i.stock !== null ? ` [stock: ${i.stock}]` : ""}${i.has_image ? ` [id:${i.id}, tiene foto disponible]` : ""}`)
    .join("\n");
...
- Los productos que muestran "[stock: N]" en el catálogo tienen inventario limitado a esa cantidad — nunca ofrezcas ni confirmes una cantidad mayor a la que hay en stock. Esto aplica SIEMPRE que el cliente indique una cantidad, sin importar cómo la haya dado: en el mismo mensaje donde pide el producto, o como respuesta corta a una pregunta tuya anterior (por ejemplo, si preguntaste "¿Cuántos kilos quieres?" y el cliente responde solo "10", igual debes revisar ese número contra el stock antes de confirmar — no lo aceptes automáticamente solo porque es la respuesta directa a tu pregunta). Si el cliente pide más de lo disponible, dile honestamente cuánto tienes ("Por ahora tengo N disponibles") y ofrécele ajustar la cantidad o consultar cuándo habrá más, sin inventar que vas a "consultar con el productor" u otra excusa que no te hayan indicado. Los productos sin "[stock: N]" no tienen inventario controlado — puedes asumir que siempre hay disponibles.
...
- Responder siempre en el mismo idioma en que te escribe el cliente. Si te escribe en español, usa español neutro, amable pero sin modismos chilenos informales (nada de "bacán", "al tiro", "cachai", "po") y sin sonar tampoco excesivamente formal o robótico. Si te escribe en otro idioma, responde de forma natural en ese idioma, manteniendo el mismo tono amable y profesional. Mantén las respuestas razonablemente breves, salvo que las instrucciones del negocio (más arriba) pidan un estilo más extenso, como explicaciones o recomendaciones detalladas. Sin markdown ni asteriscos.
...
async function processMessage(business, sessionId, message) {
  ...
  const { rows: menuItems } = await pool.query(
    "select id, name, price, stock, image_data is not null as has_image from menu_items where business_id = $1 and active = true order by category, name",
    [business.id]
  );
